...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    jQuery.getJSON(resultsData, function(candidates) {
  ...
  var violationsdata = "https://spreadsheets.google.com/feeds/list/1MXr_Y4RX8BEP_aBvr02_pEFNtWJAeTqfloOjQSWn6ys/od6/public/values?alt=json";
	jQuery.getJSON(violationsdata, function(json) {
    ...
    jQuery.getJSON(endorsementsdata, function(data) {
    ...
    jQuery.getJSON(gallerydata, function(galleryjson) {
        ...
        var jQuerygallery = jQuery('.gallery').flickity({
        cellSelector: 'img',
        imagesLoaded: true,
    ...
    var jQuerycaption = jQuery('.caption');
    // Flickity instance
    var flkty = jQuerygallery.data('flickity');

    jQuerygallery.on('cellSelect', function() {
        // set image caption using img's alt
        jQuerycaption.text(flkty.selectedElement.alt)
    });
});
...
function setSidebar() {
	jQueryel = jQuery("#scrollbar");
	if (jQuery(this).scrollTop() > 600 && jQueryel.css('position') != "fixed") {
		jQueryel.css({"position": "fixed", "top": "80px"});
	}
	if (jQuery(this).scrollTop() < 600 && jQueryel.css('position') == "fixed") {
		jQueryel.css({"position": "relative", "top": "0px"});
	}
}
...
function clean_google_sheet_json(data){
	...
	jQuery.each(data.feed.entry, function(i, entry) {
		...
		jQuery.each(entry, function(key, value){
			// fields that were in the spreadsheet start with gsxjQuery
			if (key.indexOf("gsxjQuery") == 0)
			{
				// get everything after gsxjQuery
				real_keyname = key.substring(4);
				elem[real_keyname] = value['jQueryt'];
			}
		});
