...
-->
			...
			<?php $i=0 ; foreach ($two_cents_contributors as $contributor): ?>
			<div class="medium-3 columns hide-for-medium hide-for-small"> <img data-tooltip class="has-tip tip-left" title="<?php echo $contributor['name']?></br><?php echo $contributor['position']?>" src=<?php echo $contributor[ 'img'] ?> /> </div>
			<?php $i++; endforeach ?>
			<?php while ($i%4!=0): ?>
			<div class="medium-3 columns hide-for-medium hide-for-small"> </div>
			<?php $i++; endwhile; ?> </div>
		<div class="db-divide hide-for-medium hide-for-small"></div>
		<ul id="sidebar-list">
			<?php dynamic_sidebar('sidebar-primary'); ?> </ul>
		<!-- end div#sidebar-list -->
	</div>
	<?php else: ?>
